...
def read_config(filename):
    ...
    if os.path.isfile(filename):
        ...
        print("Configurasyon dosyası" + filename + "bulunamadı")
...
def print_to_file(filename, unique_list_finished):
    ...
    inspect = input("> aşırı hızlı baskı? (Y/n) : ").lower()
    if inspect == "y":
        try:
...
def print_cow():
    ...
    print(28 * " " + "[ St0na | https://github.com/St0na/]\r\n")
...
def version():
    ...
    print("	* St0na Tarafından kodlanmıştır - instagram: St0na")
    print("	* St0naDev@gmail.com\r\n")
    print(" * telif hakları bana aittir\r\n")
...
def improve_dictionary(file_to_open):
    ...
    print("\r\n  *************************************************")
    print("      *                    UYARI!!!                   *")
    print("      *         sisteme doğru bilgileri yazın         *")
    print("      *      sorularda Y/N yes ve no anlamına gelmektedir   *")
    print("      *************************************************\r\n")
    ...
    if conts == "y" and len(listic) > CONFIG["global"]["threshold"]:
        print(
            "\r\n[-]  Birleştirme için maksimum kelime sayısı:"
            + str(CONFIG["global"]["threshold"])
        )
        print("[-] Bu sayıyı artırmak için yapılandırma dosyasını kontrol edin.\r\n")
        conts = input(
            "> Kelime listesindeki tüm kelimeleri birleştirmek ister misiniz? Y/[N]: "
        ).lower()
    ...
    spechars = [""]
    spechars1 = input(
        "> Kelimelerin sonuna özel karakterler eklemek ister misiniz? (örnek: .,-+/\~|^€¥$) Y/[N]: "
    ).lower()
    if spechars1 == "y":
        for spec1 in chars:
    ...
    randnum = input(
        "> şifrelerin sonuna rasgele sayılar eklemek istermisiniz (şansı arttırır) Y/[N]:"
    ).lower()
    leetmode = input("> leet modu açık olsunmu?? (örnek: leet = 1337) Y/[N]: ").lower()
    ...
    print("\r\n[+] şimdi şifreleri yapıyorum...")

    print("[+] oluşturulan şifreleri düzeltiyorum...")
...
def interactive():
    """-İ anahtarının uygulanması. Kullanıcıyı etkileşimli olarak sorgulayın ve
    cevaba göre bir şifre sözlüğü dosyası oluşturmaya yarar"""

    print("\r\n[+] şifre listesi yapmak için hedefle ilgili bilgileri yazın")
    ...
    name = input("> hedefin ismi nedir?: ").lower()
    while len(name) == 0 or name == " " or name == "  " or name == "   ":
        print("\r\n[-] bir isim girmelisiniz ")
        name = input("> hedefin ismi nedir?: ").lower()
    profile["name"] = str(name)

    profile["surname"] = input("> Soyismi nedir?: ").lower()
    profile["nick"] = input("> takma nicki nedir: ").lower()
    birthdate = input("> hedefin doğum tarihi nedir? (DDMMYYYY): ")
    while len(birthdate) != 0 and len(birthdate) != 8:
        print("\r\n[-] eksik veya hatalı bir yazım yaptın!")
        birthdate = input("> hedefin doğum tarihi nedir (DDMMYYYY): ")
    profile["birthdate"] = str(birthdate)

    print("\r\n")

    profile["wife"] = input("> partnerinin ismi nedir: ").lower()
    profile["wifen"] = input("> partnerinin soyismi nedir: ").lower()
    wifeb = input("> partnerinin doğum tarihi nedir (DDMMYYYY): ")
    while len(wifeb) != 0 and len(wifeb) != 8:
        print("\r\n[-] eksik veya hatalı doğum tarihi yazdın!")
        wifeb = input("> partnerinin doğum tarihi (DDMMYYYY): ")
    profile["wifeb"] = str(wifeb)
    print("\r\n")

    profile["kid"] = input("> çocuğunun ismi: ").lower()
    profile["kidn"] = input("> çocuğunun soymismi: ").lower()
    kidb = input("> çocuğunun doğum tarihi (DDMMYYYY): ")
    ...
    profile["pet"] = input("> hayvanının ismi nedir: ").lower()
    profile["company"] = input(">  şirketinin ismi nedir: ").lower()
    print("\r\n")

    profile["words"] = [""]
    words1 = input(
        "> daha fazla anahtar kelime eklemek istermisiniz? Y/[N]: "
    ).lower()
    words2 = ""
    if words1 == "y":
        words2 = input(
            "> lütfen hedefin şifrede kullanabileceği anahtar kelimeleri yazın [örnek: fenerbahçe,ali,pembe,baran]aralarında , olsun :"
        ).replace(" ", "")
    profile["words"] = words2.split(",")

    profile["spechars1"] = input(
        "> anahtar kelimelerin sonuna özel karakterler eklensinmi? Y/[N]: "
    ).lower()

    profile["randnum"] = input(
        "> anahtar kelimelerin sonuna rastgele sayılar eklensinmi? Y/[N]:"
    ).lower()
    profile["leetmode"] = input("> Leet modu açık olsunmu? (örnek: leet = 1337) Y/[N]: ").lower()

    generate_wordlist_from_profile(profile)  # generate the wordlist


def generate_wordlist_from_profile(profile):
    """ verdiğiniz bilgilerle wordlist oluşturuluyor... """
    ...
    print("\r\n[+] şifreler yeniden oluşturuluyor...")
    ...
    print("[+] şifreler kontrol ediliyor ve düzeltiliyor...")
...
def download_http(url, targetfile):
    print("[+] indiriliyor " + targetfile + " şu bölüme" + url + " ... ")
...
def alectodb_download():
    """alectodb kullanmak istiyorsanız local filesini ayarlayın"""

    url = CONFIG["global"]["alectourl"]

    print("\r\n[+] alectodb kontrol ediliyor...")
    ...
    print(
        "\r\n[+] tüm şifreler alectodb ye gönderiliyor[+] başarılı👌."
    )
